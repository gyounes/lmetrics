...
metrics({_Key, state, CRDT}) ->
    M = {state, lmetrics_util:size(crdt, CRDT)},
    record_message([M]);
metrics({_Key, state_driven, _From, Delta}) ->
    M = {state, lmetrics_util:size(crdt, Delta)},
    record_message([M]);
metrics({_Key, digest_driven, _From, _Bottom, Digest}) ->
    M = {digest, lmetrics_util:size(term, Digest)},
    record_message([M]);
metrics({_Key, digest_driven_with_state, _From, Delta, Digest}) ->
    M1 = {state, lmetrics_util:size(crdt, Delta)},
    M2 = {digest, lmetrics_util:size(term, Digest)},
    record_message([M1, M2]);
metrics({_Key, delta, _From, Sequence, Delta}) ->
    M = {delta, lmetrics_util:size(term, Sequence) + lmetrics_util:size(crdt, Delta)},
    record_message([M]);
metrics({_Key, delta_ack, _From, Sequence}) ->
    M = {delta_ack, lmetrics_util:size(term, Sequence)},
    record_message([M]);
metrics({tcbcast, _Actor, MessageBody, MessageVClock, Sender}) ->
    M= {tcbcast, lmetrics_util:size(MessageBody) + lmetrics_util:size(MessageVClock) + lmetrics_util:size(Sender)};
metrics({tcbcast_ack, MessageVClock, _Actor}) ->
    M= {tcbcast_ack, lmetrics_util:size(MessageVClock)}.
